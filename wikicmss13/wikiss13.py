...
class Wikiss13(commands.Cog):
    ...
    def __init__(self):
        self.config = Config.get_conf(self, identifier=9325940573)
    ...
    async def set_wiki_url(self, ctx, text):
        ...
        try:
            await self.config.guild(ctx.guild).wiki_url.set(text)
        ...
        async with ctx.typing():
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    wiki_url + api_path, params=payload, headers=self.HEADERS
                ) as res:
